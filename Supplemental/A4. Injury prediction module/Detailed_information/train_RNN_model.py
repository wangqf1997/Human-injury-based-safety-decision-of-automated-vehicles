...
def main():
    ''' Train and test the RNN-based occupant injury prediction model. '''
    ...
    Flag_LSTM = [True, False]
    Bidirectional = [True, False] * 2
    Batch_Size = [256, 128, 64] * 4
    In_Hid_dim = [(16, 16), (16, 32), (32, 64)] * 12
    Layer_ = [2, 1] * 36
    Dropout = [0.2, 0.5] * 72
    Learning_Rate = [0.0001, 0.0003, 0.001, 0.003, 0.01] * 144

    # Start the grid search for the optimal parameter combination.
    for num_mesh in range(720):
        # Obtain parameters.
        flag = Flag_LSTM[num_mesh // 360]
        bidirectional = Bidirectional[num_mesh // 180]
        Batch_size = Batch_Size[num_mesh // 60]
        (In_dim, Hid_dim) = In_Hid_dim[num_mesh // 20]
        Layer = Layer_[num_mesh // 10]
        dropout = Dropout[num_mesh // 5]
        Learning_rate = Learning_Rate[num_mesh]

        if opt.print_inf:
            print('\nGrid search epoch: ', num_mesh + 1, '/720')
            print('Parameters: ', flag, bidirectional, Batch_size, In_dim, Hid_dim, Layer, dropout, Learning_rate)
            ...
            if (epoch + 1) % 1 == 0:
                ...
                if opt.save_para:
                    torch.save(model.state_dict(),
                               'Saved_Model_params/RNN_%s %s %s.pkl' % (opt.re_samp, num_mesh + 1, epoch + 1))
        ...
        if opt.save_loss:
            ...
            plt.legend()
            plt.savefig('Prediction_IMAGE/Loss RNN_%s %s.png' % (opt.re_samp, num_mesh + 1))
            plt.close()
